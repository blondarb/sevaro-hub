...
export async function POST(
  ...
  try {
    ...
    // Atomicity fix (Codex round-2 H#2): downstream FIRST, source-of-truth
    // LAST. Today's PR (Codex M#4 first pass) reordered to PATCH-first, but
    // that puts the SoT mutation (clearing triageProposal, flipping to
    // in_progress) ahead of the queue write — so if the queue write fails
    // after the PATCH, the proposal is gone and retries hit the "no triage
    // proposal" 404 path even though the stable promptId is unchanged. New
    // order:
    //
    //   1. Create queue item with stable promptId = improvement-<sid>-v<ver>.
    ...
    //      If this fails, nothing else runs; the proposal is still on the
    //      session and a retry is clean.
    //   2. Write triage history (append-only audit). If this fails after the
    //      queue write, a retry may produce a duplicate history row — that is
    //      acceptable: the paper trail correctly shows both attempts. The
    //      queue row is stable (step 1 upserts), and the SoT is still intact.
    //   3. PATCH session: clear triageProposal, flip to in_progress. This is
    //      the real "commit" — only happens once steps 1+2 succeeded. If this
    //      fails, the next retry re-runs 1 (no-op) + 2 (dup) + 3 (success).
