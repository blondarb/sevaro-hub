...
const MAX = 100;
const fields = ['item_id','source_url','source_revision','first_observed_at','last_observed_at','packet_digest','run_id'];
const hash = value => typeof value === 'string' && /^[a-f0-9]{64}$/.test(value);

export async function prepareReplyRecheck(packets, {allowedHosts, now=Date.now(), prior=null}={}) {
  ...
  if (prior) {
    ...
    requireThat(prior.schema_version===1 && prior.purpose===PURPOSE && prior.source_id==='claude:replies' &&
    ...
    for (const ref of prior.references) {
      ...
      requireThat(typeof ref.source_revision==='string' && ref.source_revision.length>0 && ref.source_revision.length<=100 &&
        hash(ref.packet_digest) && isAllowedSourceUrl(ref.source_url,allowedHosts) &&
  ...
  for (const {feed,receipt} of reviewed) {
    for (const item of feed.items) {
      ...
      const ref={item_id:item.item_id,source_url:item.source_url,source_revision:item.source_revision,
        first_observed_at:feed.observed_at,last_observed_at:feed.observed_at,packet_digest:receipt.packet_digest,run_id:receipt.run_id};
      if (previous) {
  ...
  const body={schema_version:1,purpose:PURPOSE,generated_at:new Date(now).toISOString(),source_id:'claude:replies',
