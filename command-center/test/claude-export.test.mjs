...
import {assemble} from '../context.mjs';
...
test('old reply evidence is capped at two hours without changing the reviewed packet',async()=>{
 ...
 p.run.started_at=p.feed.observed_at;p.run.completed_at=new Date(NOW).toISOString();p.review.reviewed_at=new Date(NOW).toISOString();p.review.feed_digest=await sha256(p.feed);
