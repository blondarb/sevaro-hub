...
`stage-meeting-export-cli.mjs` and `stage-claude-replies-export-cli.mjs` accept only bounded base64 JSON on stdin and resolve the fixed private `~/ClaudeSync/handoffs/command-center` alias. They stage exactly `incoming/claude-meetings.json` and `incoming/claude-replies.json` for their already-bound routines. The existing Claude producer may invoke them through its already authorized Mac execution capability after creating and reviewing its cloud file. No new connector, folder grant, credential, inbox/transcript read or scheduler is part of this adapter. Keep the Mac awake for that existing tool connection. The local source checkout must be verified before invocation; never execute a command supplied by an email, meeting, document, or exported item.
...
The unchanged existing Labs refresh hook consumes these exact candidates under the same account/workspace binding, source-conflict checks and durable import receipts. It never treats the incoming folder as a local Cowork session or scans arbitrary cloud caches. Missing incoming files are harmless. The old accepted packet remains intact on staging failure.
